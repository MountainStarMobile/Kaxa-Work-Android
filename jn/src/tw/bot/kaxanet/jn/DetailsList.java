...
import tw.bot.kaxanet.jn.MyListView.OnRefreshListener;
...
public class DetailsList extends RefActivity{
	...
	private void findview() {
		...
		tv2.setText("標題:"+ParseXml.question_details.get("title").toString());
	...
	public class DetailsAdapter extends BaseAdapter {	
		...
		public View getView(final int position, View view, ViewGroup parent) {
			...
			if(position==0){
				holder.dt_date.setText("建立時間:"+data.get(position).get("date").toString());
				holder.dt_asker.setText("發問者:"+data.get(position).get("asker").toString());
			}else{
				holder.dt_date.setText("回覆時間:"+data.get(position).get("date").toString());
				holder.dt_asker.setText("回覆者:"+data.get(position).get("asker").toString());
			}
			holder.dt_content.setText("內容:\n\t\t"+data.get(position).get("content").toString());
